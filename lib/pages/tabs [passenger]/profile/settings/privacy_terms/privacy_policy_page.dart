...
//  Two-bar header:
//  ┌─────────────────────────────────────────────┐
//  │  ←   Privacy Policy                         │  ← SliverAppBar (pinned)
//  ├─────────────────────────────────────────────┤
//  │  [logo] Moviroo              [🌐 EN ▾]      │  ← Branded sub-header
//  └─────────────────────────────────────────────┘
//
//  Supported languages: en, fr, ar, de, es, it, pt, tr
//  Loads from: data/privacy/privacy_patch_{lang}.json
...
class _PrivacyPolicyPageState extends State<PrivacyPolicyPage> {
  ...
  void didChangeDependencies() {
    ...
    if (_selectedLang == null) {
      ...
      _loadPrivacy(_selectedLang!);
    }
  }

  Future<void> _loadPrivacy(String code) async {
    if (!mounted) return;
    setState(() { _privacy = null; _hasError = false; });
    try {
      final raw  = await rootBundle
          .loadString('data/privacy/privacy_patch_$code.json');
      final full = json.decode(raw) as Map<String, dynamic>;
      final data = full.containsKey('privacy')
          ? full['privacy'] as Map<String, dynamic>
          : full;
      if (mounted) setState(() => _privacy = data);
    } catch (e) {
      debugPrint('PrivacyPolicyPage load error: $e');
  ...
  Widget _buildContent(BuildContext context) {
    ...
    if (_privacy == null && !_hasError) {
      return _loadingScaffold(context, lang, isRtl);
    }
    if (_hasError) {
      return _errorScaffold(context, lang, isRtl);
    }

    final meta     = (_privacy!['meta']     as Map<String, dynamic>?) ?? {};
    final sections = (_privacy!['sections'] as List<dynamic>?)        ?? [];
  ...
  Widget _loadingScaffold(BuildContext context, String lang, bool isRtl) {
    return Scaffold(
      backgroundColor: AppColors.lightBg,
      body: Column(
        children: [
  ...
  Widget _errorScaffold(BuildContext context, String lang, bool isRtl) {
    return Scaffold(
      backgroundColor: AppColors.lightBg,
      body: Column(
        children: [
              ...
              child: Text(
                [
                  ...
                  if ((meta['effectiveDate'] as String? ?? '').isNotEmpty)
                    '${legalLabel(LegalLabelKey.effectiveAs, lang)} ${meta['effectiveDate']}.',
                ].join('\n'),
