...
import 'package:moviroo/pages/search/voice/api_service.dart';
import 'package:moviroo/pages/search/voice/constants.dart';
import 'package:moviroo/pages/search/voice/logger.dart';
import 'package:moviroo/pages/search/voice/voice_phase.dart';
...
class _VoiceAssistantScreenState extends State<VoiceAssistantScreen>
  ...
  void initState() {
    super.initState();
    appLog('INIT', 'VoiceAssistantScreen mounted — backend: $kBackendUrl');

    // Pulse du bouton micro
  ...
  Future<void> _initTts() async {
    ...
    _tts.setCompletionHandler(() {
      appLog('TTS', 'Speech finished → starting answer recording');
  ...
  Future<void> _speak(String text, String lang) async {
    ...
    appLog('TTS', 'speak  lang=$locale  →  "$text"');
  ...
  Future<void> _startRecording({bool isAnswer = false}) async {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (t) {
      ...
      if (_elapsed.inSeconds >= kMaxSeconds) {
        appLog('REC', 'Max duration reached (${kMaxSeconds}s) — auto stop');
  ...
  Future<void> _stopAndTranscribe() async {
    ...
    _audioPath = path;
    appLog('REC', 'Initial recording stopped → $path');
  ...
  Future<void> _stopAndAnswer() async {
    ...
    _audioPath = path;
    appLog('REC', 'Answer recording stopped → $path  field=$_currentField');
    setState(() { _phase = VoicePhase.uploading; _statusMsg = 'PROCESSING...'; });
    try {
  ...
  Future<void> _handleResult(Map<String, dynamic> result) async {
    ...
    // ── Tous les champs remplis → confirmation ───────────────
  ...
  void _reset() {
    appLog('RESET', 'User reset — back to idle');
    setState(() {
  ...
  Widget _buildMessageBubble() {
    ...
    return Container(
      ...
      child: RichText(
        ...
        text: TextSpan(
          style:
              const TextStyle(fontSize: 15, height: 1.5, color: kTextMain),
          children: [
            ...
            if (lastWord.isNotEmpty)
              TextSpan(
                text: lastWord,
                style: TextStyle(
                    color: highlight, fontWeight: FontWeight.w600),
              ),
          ],
        ),
      ),
